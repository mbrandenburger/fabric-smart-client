...
func (client *Client) parseSecureOptions(opts SecureOptions) error {
	...
	// A client presents its keypair only when mutual TLS is asked for. TLSConfig loads any
	// keypair it is handed, because for a listener that keypair is the server certificate —
	// so the client side has to withhold it here. tlsconfig.buildClient clears the same two
	// fields for the same reason, which is what makes an explicit clientAuthEnabled:false a
	// way to say "do not present a certificate to this endpoint".
	if !opts.RequireClientCert {
		opts.Certificate, opts.Key = nil, nil
	}
