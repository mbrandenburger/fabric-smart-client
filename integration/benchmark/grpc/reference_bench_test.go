...
func BenchmarkRef(b *testing.B) {
	...
	client, closeF := setupRefClient(b)
	defer closeF()
...
func setupRefServer(tb testing.TB) {
	tb.Helper()
...
func setupRefClient(tb testing.TB) (protos.ViewServiceClient, func()) {
	tb.Helper()
	...
	return protos.NewViewServiceClient(conn), func() { conn.Close() }
}
